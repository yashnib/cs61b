...
public class TestOneWordK0Hyponyms {
    ...
    @Test
    public void testActK0MultipleQueries(){
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                WORDS_FILE, TOTAL_COUNTS_FILE, SMALL_SYNSET_FILE, SMALL_HYPONYM_FILE);
        List<String> words = new ArrayList<>();
        words.add("act");
        ...
        String expected0 = "[act, action, change, demotion, human_action, human_activity, variation]";
        ...
        String expected1 = "[act, action, change, demotion, human_action, human_activity, variation]";
        assertThat(actual1).isEqualTo(expected1);
    }

    @Test
    public void testActKNot0SingleQuery(){
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                WORDS_FILE, TOTAL_COUNTS_FILE, SMALL_SYNSET_FILE, SMALL_HYPONYM_FILE);
        List<String> words = new ArrayList<>();
        words.add("act");

        NgordnetQuery nq0 = new NgordnetQuery(words, 1480, 1485, 4);

        String actual0 = studentHandler.handle(nq0);
        String expected0 = "[act, action, change, demotion]";
        assertThat(actual0).isEqualTo(expected0);
    }

    @Test
    public void testActKNot0MultipleQueries(){
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                WORDS_FILE, TOTAL_COUNTS_FILE, SMALL_SYNSET_FILE, SMALL_HYPONYM_FILE);
        List<String> words = new ArrayList<>();
        words.add("act");
        ...
        NgordnetQuery nq1 = new NgordnetQuery(words, 1472, 1485, 4);
        String actual1 = studentHandler.handle(nq1);
        String expected1 = "[act, action, change, variation]";
        assertThat(actual1).isEqualTo(expected1);
    }
}
