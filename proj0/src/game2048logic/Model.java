...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                Tile curTile = board.tile(i, j);
                if (curTile == null) {
                    continue;
                }
                int curTileVal = curTile.value();

                int east = j - 1;
                int west = j + 1;
                if (east >= 0 && board.tile(i, east).value() == curTileVal){
                    return true;
                }
                else if (west < size && board.tile(i, west).value() == curTileVal){
                    return true;
                }

                int north = i + 1;
                int south = i - 1;
                if (south >= 0 && board.tile(south, j).value() == curTileVal){
                    return true;
                }
                else if (north < size && board.tile(north, j).value() == curTileVal){
                    return true;
                }
            }
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int size = size();
        if (!atLeastOneMoveExists() || y == size - 1){
            return;
        }

        y = y + 1;
        Tile upperTile = board.tile(x, y);
